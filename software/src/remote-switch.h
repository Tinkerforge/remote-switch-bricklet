 ...
 *
 * remote-switch.h: Implementation of Remote Switch Bricklet messages
 *
 * Register definitions from felix@lowpowerlab.com (C) 2013:
...
#define REG_ENDOFCONFIG       0xFF // To signify when end of config is reached
...
typedef struct {
	...
	uint8_t switch_to;
} __attribute__((__packed__)) SwitchSocket;
...
typedef struct {
	MessageHeader header;
	uint8_t state;
} __attribute__((__packed__)) GetSwitchingStateReturn;

typedef struct {
	MessageHeader header;
} __attribute__((__packed__)) SwitchingDone;
